...
public class GridHeaderDisplayNameBehavior : Behavior<DataGrid>
{
    ...
    {
        if (e.Column.Header.ToString() is not { } propertyName) return;

        e.Column.Header = ((DataView)AssociatedObject.ItemsSource).Table?.Columns[int.Parse(propertyName)].Caption;
